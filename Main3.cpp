...
*/
    ...
    {
    	for (int j = 0; j<5; j++)
    		{cout<< "day "  << i+1  << " combination 1  estimated data: "  << esti_data[i][j]<< endl;
    		if (esti_data[i][j] > max_temp)  max_temp = esti_data[i][j];
    		if (esti_data[i][j] < min_temp)   min_temp = esti_data[i][j];
    		if ( (max_temp - min_temp) < 100000000  && (max_temp-min_temp) > 800)
    			cout<< "day "  << i+1  << " combination 1 "  << "anomalous"<< endl;
    		}
    }
