...
int main(int argc, char** argv) {
	...
	for (int running_time = 0; running_time < 1; running_time++) {
		...
		ofstream pFile;
		pFile.open(
				"/export/qhome/zyu/intersect/zhenglin/data/src/Four_8_experiments/buckets1024_s1.txt.txt.txt",
				ios::app);
		...
		int total = 4096;
		int buckets = 1024, depth = 1; 
		int s1 = total/buckets;
		...
		for (int reg = 0; reg < num_regions; ++reg) {
			...
			string
					sample_filename =
							"/export/qhome/zyu/intersect/zhenglin/data/src/Four_8_experiments/test_i_R_one_percent.txt";
			string line;
