...
int main(int argc, char** argv) {
         ofstream pFile;
         pFile.open("/export/qhome/zyu/intersect/zhenglin/data/src/myfile.txt",  ios::app);
          ...
          //  fputs ("fopen example\n",pFile);
           pFile << "Start running\n";
            ...
            pFile << "day test " + intToStringPad(test1, 4) + " combination test  " +intToStringPad(test2, 4)  + " " + "anomolous \n";


pFile.close();
pFile.open("/export/qhome/zyu/intersect/zhenglin/data/src/myfile.txt",  ios::app);
        int recognized = 0;
        int sum_running_times = 0;


        for (int running_time =0; running_time < 100; running_time++)
        {
                ...
                for (int i = 0; i < 12; i++) {
                        max_temp[i] = -100000000;
                        min_temp[i] = 100000000;
                }
                ...
                if (argc > 1)
                       ...
                       cout << "buckets = " << buckets << ", s1 = " << s1 << ", depth = " << depth
                                       << endl;
                       ...
                       string sample_filename =
                                       "/export/qhome/zyu/intersect/zhenglin/Combinations_Different_Approach2_500MS_with_big_changes_dates_Tue_Dec.txt";
                       ...
                       for (int i = 0; i < numPairs; ++i) {
                                      cout << " pair1 i=  " << i << "  " << pair1[i] << ", " << " pair2 i=  "
                                                      << i << "  " << pair2[i] << endl;
                              }
 ...
 vector<long long> realF0(numPairs, 0);
        ...
        for (day = 1; day <= 5; day++) {
                        ...
                        epoch.tm_min = 0;
                                                ...
                                                igzstream in(filename.c_str());
                                                                      ...
                                                                      while (getline(in, line)) {
                                                                              ...
                                                                              //cout << count << " " << v_F0 << ", " << value << "  " << v_F1 << endl;                                //if (value < 0) { cout << tokens[21].c_str() << "|" << tokens[22].c_str() << endl; exit(1); }
                ...
                for (int i = 0; i < numPairs; ++i) {
                                       ...
                                       cout << "abF1 " << abF1 << " " << "aF1 " << aF1 << " " << "bF1 "
                                                       << bF1 << endl;
                                       long double estF1 = compute_intersect(abF1, aF1, bF1);

                                       cout << "pair1[i] " << pair1[i] << "|" << "pair2[i] " << pair2[i]
                                                       << endl;
                                       cout << "realF0[i] " << realF0[i] << " " << "realF1 " << realF1[i]
                                                       << endl;
                                       ...
                                       esti_data[i][day - 1] = estF1 / estF0;
                                                              if (esti_data[i][day - 1] < 0)  esti_data[i][day - 1] = - esti_data[i][day - 1];

                                                              cout << "day " << day << " combination " << i << " "
                                                                              << esti_data[i][day - 1] << endl;
                                                              if (esti_data[i][day - 1] > max_temp[i])
                                                                      max_temp[i] = esti_data[i][day - 1];
                                                              if (esti_data[i][day - 1] < min_temp[i])
                                                                      min_temp[i] = esti_data[i][day - 1];
                                                              ...
                                                              AMS_Destroy(sketchSumF0);
                                                                      ...
                                                                      for (int i = 0; i < 12; i++){
                                                                              cout << max_temp[i]<<"  " << min_temp[i] << endl;
                                                                              if ((max_temp[i] - min_temp[i]) < 1000000
                                                                                              && (max_temp[i] - min_temp[i]) > 800)

                                                                                      {cout << "day " << day << " combination  " << i << " "
                                                                                                      << "anomolous" << endl;

                                                                                      pFile << "day " + intToStringPad(day, 2) + " combination  " + intToStringPad(i, 2) + " " + "anomolous \n";
                                                              recognized +=1;
                                                              sum_running_times +=1;
        ...
        pFile<< "recognized " + intToStringPad(recognized, 4) + " sum_running_times  " + intToStringPad(sum_running_times, 4) + " \n";
       pFile.close();
                         }
               return 0;
       }
