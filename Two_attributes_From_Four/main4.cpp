...
double esti_data[12][5] = {};
double max_temp =- 100000000;
double min_temp = 100000000;
  ...
  string region = "NC/ILNWI";
    ...
    for (int i = 0; i < numPairs; ++i)
           {
    	cout  << " pair1 i=  "  << i << "  "<<  pair1[i] << ", " << " pair2 i=  "  << i << "  "<<  pair2[i] << endl;
           }

    /*
    bool inFirst = (tokens[1] + "|" + tokens[17] == pair1[i]);
             bool inSecond = (tokens[10] + "|" + tokens[18] == pair2[i]);
*/
 ...
 /* for(int reg = 0; reg < 1; ++reg)
  {
    ...
    //
    string sample_filename = intToString(reg + 1) + "-" + intToStringPad(year, 4) + "-" + intToStringPad(month, 2) + "-" +intToStringPad(day, 2) + "-sample.txt";
    string line;
    ifstream sample_file (sample_filename.c_str());
    while (getline (sample_file, line))
    {
...
// update data.for 24 hours of the day
    ...
    for (int i = 0; i < numPairs; ++i)
    {
     ...
     esti_data[i][day-1] =  estF1/  estF0;

				cout<< "day "  << day << " combination "  << i  << " "<< esti_data[i][day-1]<< endl;
    		if (esti_data[i][day-1] > max_temp)  max_temp = esti_data[i][day-1];
    		if (esti_data[i][day-1] < min_temp)   min_temp = esti_data[i][day-1];
    		if ( (max_temp - min_temp) < 1000000  && (max_temp-min_temp) > 800)
    			cout<< "day "  << day  << " combination  "<< i  << " "   << "anomolous"<< endl;


cout<< "Hello! Something is wrong!" << endl;
