/*** Code for running intersection sketch
     Author: Ashwin Lall
...
long hash_S_LL(string s)
{    // string  hashed to a long integer
  long hash = 5381;
  int c;
...
int main (int argc, char** argv)
{
 ...
 double data1[5] = {0, 0, 0, 0, 0};
 double data2[5] = {0, 0, 0, 0, 0};
 double data3[5] = {0, 0, 0, 0, 0};
 double data4[5] = {0, 0, 0, 0, 0};
 double data5[5] = {0, 0, 0, 0, 0};
 double data6[5] = {0, 0, 0, 0, 0};
 double data7[5] = {0, 0, 0, 0, 0};
 double data8[5] = {0, 0, 0, 0, 0};
 double data9[5] = {0, 0, 0, 0, 0};
 double data10[5] = {0, 0, 0, 0, 0};
 double data11[5] = {0, 0, 0, 0, 0};
 double data0[5] = {0, 0, 0, 0, 0};
  ...
  int buckets = 65536, depth = 1, s1 = 1;
  ...
  cout << "buckets = " << buckets << ", s1 = " << s1 << ", depth = " << depth << endl;
  ...
  vector<string> pair1, pair2;
    ...
    //
    string sample_filename = "/export/qhome/zyu/intersect/zhenglin/simulate_sample.txt";
  string region = "NC/ILNWI";
 ...
 /* for(int reg = 0; reg < 1; ++reg)
  {
    ...
    //
    string sample_filename = intToString(reg + 1) + "-" + intToStringPad(year, 4) + "-" + intToStringPad(month, 2) + "-" +intToStringPad(daay, 2) + "-sample.txt";
    string line;
    ifstream sample_file (sample_filename.c_str());
    while (getline (sample_file, line))
    {
...
for (day = 1; day <=5 ; day++){
    ...
    for (int hr = 0; hr < 24; ++hr)
    {
      ...
      struct tm epoch;
      epoch.tm_sec = 0; epoch.tm_min = 0; epoch.tm_hour = hr; epoch.tm_mday = day; epoch.tm_mon = month - 1; epoch.tm_year = year - 1900;
      string epoch_time = intToString(mktime(&epoch));

      //cout << region << " " << hour << " " << epoch_time << endl;
      //string filename = "/export/qhome/zyu/intersect/zhenglin/records_with_big_intersection_ratio_bigger_than_10_percent.txt";
      string filename =  base_path + "/" + region + "/FULL/" + intToStringPad(year, 4) + "/" + intToStringPad(month, 2) + "/" + intToStringPad(day, 2) +  "/RNC." + super_region + "." + sub_region + ".PA." + epoch_time + ".dat.gz";
      ...
      {
        vector<string> tokens = split(line.c_str(), '|');
        long long count = atoi(tokens[4].c_str());
        long long value = atoi(tokens[5].c_str());
        value = value / 1000;
        if (value < 0) value = -value; // prevents overflow but still gives a reasonable update
        ...
        //cout << count << " " << v_F0 << ", " << value << "  " << v_F1 << endl;
        //if (value < 0) { cout << tokens[21].c_str() << "|" << tokens[22].c_str() << endl; exit(1); }

        for (int i = 0; i < numPairs; ++i)
        {
          bool inFirst = (tokens[0] + "|" + tokens[2] == pair1[i]);
          bool inSecond = (tokens[1] + "|" + tokens[3] == pair2[i]);
    ...
    for (int i = 0; i < numPairs; ++i)
    {
     ...
     cout << "abF0 " << abF0 << " "  << "aF0 " << aF0 << " " << "bF0 " << bF0 << endl;
     ...
     cout<< "abF1 "  << abF1 << " " << "aF1 " << aF1 << " " << "bF1 " << bF1 << endl;
      long double estF1 = compute_intersect(abF1, aF1, bF1);

     cout<< "pair1[i] "  << pair1[i] << "|"<< "pair2[i] "  << pair2[i] << endl;
     cout<< "realF0[i] "  << realF0[i] << " "<< "realF1 "  << realF1[i] << endl;
     cout << "estF0 " << estF0 << " "<< "estF1 "  << estF1 << endl;
     cout << endl;

     if (numPairs == 0 )
          data0[day-1] =  estF1/  estF0;
     else if (numPairs == 1 )
                  data1[day-1] =  estF1/  estF0;
     else if (numPairs == 2 )
                       day2[day-1] =  estF1/  estF0;
     else if (numPairs == 3 )
                       day3[day-1] =  estF1/  estF0;
     else if (numPairs == 4 )
                            day4[day-1] =  estF1/  estF0;
     else if (numPairs == 5 )
                            day5[day-1] =  estF1/  estF0;
     else if (numPairs == 6 )
                            day6[day-1] =  estF1/  estF0;
     else if (numPairs == 7 )
                            day7[day-1] =  estF1/  estF0;
     else if (numPairs == 8 )
                            day8[day-1] =  estF1/  estF0;
     else if (numPairs == 9 )
                            day9[day-1] =  estF1/  estF0;
     else if (numPairs == 10 )
                            day10[day-1] =  estF1/  estF0;
     else
                            day11[day-1] =  estF1/  estF0;
     ...
     /*
      ...
      cout << "intersection ratio: " << ir << "\t" << "errors F0: " << fabs(realF0[i] - estF0)/realF0[i] << "\t" << "errors F1 " << fabs(realF1[i] - estF1)/realF1[i] << "\t" << "errors F2 " << fabs(realAvg - estF1/estF0)/realAvg << endl;
*/


         /* end code for plotting error vs intersection rate */

      //cout << fabs(realF0[i] - estF0)/realF0[i] << " " << fabs(realF1[i] - estF1)/realF1[i] << " " << fabs(realAvg - estF1/estF0)/realAvg << endl;
    ...
    for (int i = 0; i < numPairs; ++i)
    {
      AMS_Destroy(sketch1F0[i]); AMS_Destroy(sketch1F1[i]);
      AMS_Destroy(sketch2F0[i]); AMS_Destroy(sketch2F1[i]);
    }
