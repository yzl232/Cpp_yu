...
int main(int argc, char** argv) {
	for (int running_time = 0; running_time < 1; running_time++) {
		ofstream pFile;
		pFile.open(
				"/export/qhome/zyu/intersect/zhenglin/data/src/new_redo_experiments.txt",
				ios::app);
		if (pFile != NULL) {
			// fputs ("fopen example\n",pFile);
			pFile << "Start running\n";
		...
		for (int reg = 0; reg < num_regions; ++reg) {
			...
			string sample_filename = "heavypairs-0.06-ONEREGION-"
					+ intToStringPad(year, 4) + "-" + intToStringPad(month, 2)
					+ "-" + intToStringPad(day, 2) + ".txt";
			string line;
			...
			for (int i = 0; i < numPairs; ++i) {
				...
				pFile << ir << "\t" << fabs(realF0[i] - estF0) / realF0[i]
						<< "\t" << fabs(realF1[i] - estF1) / realF1[i] << "\t"
