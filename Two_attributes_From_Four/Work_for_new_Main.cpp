...
int main(int argc, char** argv) {
	...
	for (int running_time = 0; running_time < 1; running_time++) {
		...
		ofstream pFile;
		pFile.open(
				"/export/qhome/zyu/intersect/zhenglin/data/src/redo_experiments_1024.txt",
				ios::app);
		...
		for (int reg = 0; reg < num_regions; ++reg) {
			...
			while (getline(sample_file, line)) {
				...
				pair1.push_back(tokens[1] + "|" + tokens[17]); // RNC and service
				pair2.push_back(tokens[10] + "|" + tokens[18]); // item_ID and domain
			}
			sample_file.close();
			...
			for (int i = 0; i < numPairs; ++i) {
				...
				pFile.close();
				pFile.open("/export/qhome/zyu/intersect/zhenglin/data/src/redo_experiments_1024.txt", ios::app);
