...
int main(int argc, char** argv) {
	...
	for (int running_time = 0; running_time < 1; running_time++) {
		...
		ofstream pFile;
		pFile.open(
				"/export/qhome/zyu/intersect/zhenglin/data/src/ratio_1_percent_results_4096.txt.txt",
				ios::app);
		...
		for (int reg = 0; reg < num_regions; ++reg) {
			...
			string sample_filename =	"/export/qhome/zyu/intersect/zhenglin/data/src/intersection_ratio_1_percent.txt";
			string line;
			...
			for (int i = 0; i < numPairs; ++i) {
				...
				pFile.close();
				pFile.open("/export/qhome/zyu/intersect/zhenglin/data/src/ratio_1_percent_results_4096.txt.txt", ios::app);
