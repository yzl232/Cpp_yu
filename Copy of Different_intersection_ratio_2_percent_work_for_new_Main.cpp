...
int main(int argc, char** argv) {
	...
	for (int running_time = 0; running_time < 1; running_time++) {
		...
		ofstream pFile;
		pFile.open(
				"/export/qhome/zyu/intersect/zhenglin/data/src/ratio_two_percent_results_65536.txt.txt",
				ios::app);
		...
		for (int reg = 0; reg < num_regions; ++reg) {
			...
			string sample_filename =	"/export/qhome/zyu/intersect/zhenglin/data/src/intersection_ratio_2_percent.txt";
			string line;
			...
			for (int hr = 0; hr < 24; ++hr) {
				...
				string filename = base_path + "/" + region + "/FULL/"
						...
						2) + "/" + intToStringPad(day, 2) + "/RNC."
						+ super_region + "." + sub_region + ".PA." + epoch_time
						+ ".dat.gz";
				...
				while (getline(in, line)) {
					...
					for (int i = 0; i < numPairs; ++i) {
						bool inFirst = (tokens[1] + "|" + tokens[17]
								== pair1[i]);
						bool inSecond = (tokens[10] + "|" + tokens[18]
								== pair2[i]);
			...
			for (int i = 0; i < numPairs; ++i) {
				...
				pFile << "intersection ratio: " << ir << "\t" << "errors F0: "
						...
						<< "\t" << "errors F1/F0 "
						<< fabs(realAvg - estF1 / estF0) / realAvg << endl;
				...
				pFile.close();
				pFile.open("/export/qhome/zyu/intersect/zhenglin/data/src/ratio_two_percent_results_65536.txt.txt", ios::app);
